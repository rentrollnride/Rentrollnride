...
async function sendAgreement(args: {
  ...
  const payload: Record<string, unknown> = {
    ...
    custom_fields: [
      { name: "RenterName", value: args.customer.name },
      { name: "RenterEmail", value: args.customer.email },
      { name: "RenterPhone", value: args.customer.phone },
      { name: "Vehicle", value: `${args.vehicle.year} ${args.vehicle.make} ${args.vehicle.model}` },
      { name: "Pickup", value: dateLabel(args.pickupAt) },
      { name: "Return", value: dateLabel(args.expectedReturnAt) },
      { name: "Rate", value: `${money(args.rate)} ${args.rateType}` },
      { name: "Deposit", value: money(args.deposit) },
      { name: "ApprovedTravelArea", value: APPROVED_TRAVEL_AREA },
    ],
